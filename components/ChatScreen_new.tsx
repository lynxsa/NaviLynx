...
export function ChatScreen({ venueId, onClose }: ChatScreenProps) {
  ...
  // Memoize initializeChatRoom to ensure stable reference for useEffect
  const initializeChatRoom = useCallback(async () => {
    try {
      ...
      Alert.alert('Error', `Failed to load chat room: ${error}`);
    } finally {
      setLoading(false);
    }
  }, [venueId]);
