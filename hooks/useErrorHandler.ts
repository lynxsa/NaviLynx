...
// Simple AppError interface for the hook
...
export function useErrorHandler(options: UseErrorHandlerOptions = {}): UseErrorHandlerReturn {
  ...
  const handleError = useCallback(async (
    ...
    // Log error
    // Log error (could integrate with external service later)
  ...
  }, [context, autoRetry, maxRetries, showUserFeedback, onError]);
  ...
  const retry = useCallback(async () => {
    ...
    try {
      ...
      await handleError(
        ...
        `${context} - Retry ${retryCountRef.current}`
      );
    } finally {
      setIsLoading(false);
    }
  }, [context, handleError, onRecovery]);
  ...
  }, [context, handleError, clearError, error]);
    ...
    return handleAsyncOperation(
      async () => {
        if (requiresPermission) {
          ...
          const hasPermission = location !== null;
  ...
  const handleCameraRequest = useCallback(async (
    ...
    return handleAsyncOperation(
      async () => {
        if (requiresPermission) {
          // Check camera permission (implementation depends on your camera library)
